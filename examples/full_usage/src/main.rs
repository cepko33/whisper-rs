...
fn main() {
    ...
    unsafe extern "C" fn seg_callback(
        ...
        for j in 0..num_tokens {
            let wtd = whisper_rs_sys::whisper_full_get_token_data_from_state(s,segment_id-1, j);
            let token_text = whisper_rs_sys::whisper_full_get_token_text_from_state(c, s,segment_id-1, j);
            let c_str =  CStr::from_ptr(token_text);
            let r_str = c_str.to_str().expect("Parsed");
            println!("{:?} - {:?} : {:?}", wtd.t0, wtd.t1, r_str);
        }

        //println!("{:?} {}", segment_id, num_tokens);
    }
